...
import np.com.bimalkafle.quizonline.Class.QuestionModel
...
class QuizActivity : AppCompatActivity(),View.OnClickListener {
    ...
    private fun saveScoreToFirebase(score: Int) {
        ...
        userId?.let {
            val userScoreRef = database.child("leaderboard").child(it)

            // Kiểm tra nếu người dùng đã có điểm trước đó
            userScoreRef.get().addOnSuccessListener { dataSnapshot ->
                val currentScore = dataSnapshot.child("score").getValue(Int::class.java) ?: 0
                ...
                // Cập nhật điểm mới
                userScoreRef.setValue(scoreData)
                    .addOnSuccessListener {
                        Log.d("QuizActivity", "Score updated successfully")
                    }
                    .addOnFailureListener {
                        Log.e("QuizActivity", "Failed to update score", it)
                    }
            }.addOnFailureListener {
                Log.e("QuizActivity", "Failed to retrieve current score", it)
            }
        }
    }
